...
public class hero1 : MonoBehaviour
{
    ...
    public GameObject parttt, record;
    ...
    void Start()
    {
        ...
        {
            record.GetComponent<TMPro.TextMeshPro>().text = "Train here --->" + "\r\n" + "Record:" + PlayerPrefs.GetInt("record");
        }
    }
    ...
    private void FixedUpdate()
    {
        ...
        if(!grounded && !iced)
        {
            parttt.SetActive(true);
        }
        else
        {
            parttt.SetActive(false);
        }
        if(fly == true)
        {
            fly = false;
    ...
    private void MyInput()
    {
        ...
        if (Input.GetKeyDown(KeyCode.R))
        {
            ...
            else if (SceneManager.GetActiveScene().name == "first_level")
            {
                Time.timeScale = 1;
                Time.fixedDeltaTime = Time.timeScale * 0.02f;
...
*/
    ...
    private void Movement()
    {
        ...
        if ((readyToJump && jumping && (grounded||iced) && !rocketActive)||(readyToJump && jumping && isWallRunning && !rocketActive)) Jump();
...
//            rb.AddForce((normalVector * jumpForce * 0.1f)*0.5f);
        ...
        if (!grounded && !iced)
        {
            ...
            //Add jump forces
            rb.AddForce(orientation.forward * jumpForce * 0.1f);
            rb.AddForce(Vector2.up * jumpForce * 1.5f);
    ...
    private void Dash()
    {
        ...
        if (isWallLeft)
        {
            dashStartVector = orientation.right;
    ...
    private void CounterMovement(float x, float y, Vector2 mag)
    {
        ...
        if (Mathf.Sqrt((Mathf.Pow(rb.velocity.x, 2) + Mathf.Pow(rb.velocity.z, 2))) > maxSpeed)
        {
            ...
            rb.velocity = new Vector3(n.x, 0f, n.z);
        }
    }
    ...
    public bool only_one_time = false;
    ...
    private void OnCollisionEnter(Collision collision)
    {
        if(collision.gameObject.tag == "Die")
        {
            if (SceneManager.GetActiveScene().name == "tutor")
            {
                ...
                Time.fixedDeltaTime = Time.timeScale * 0.02f;
                SceneManager.LoadScene("tutor");
            } else if (SceneManager.GetActiveScene().name == "First")
            {
                Time.timeScale = 1;
                Time.fixedDeltaTime = Time.timeScale * 0.02f;
            ...
            else if (SceneManager.GetActiveScene().name == "menu")
            {
                ...
                if (!PlayerPrefs.HasKey("record"))
                {
                    ...
                    PlayerPrefs.SetInt("record", int.Parse(GameObject.FindWithTag("kol").GetComponent<TMPro.TextMeshProUGUI>().text));
                    record.GetComponent<TMPro.TextMeshPro>().text = "Train here --->" + "/n" + "Record:" + PlayerPrefs.GetInt("record"); 
                }
                else
                {
                    if (PlayerPrefs.GetInt("record") < int.Parse(GameObject.FindWithTag("kol").GetComponent<TMPro.TextMeshProUGUI>().text))
                    {
                        PlayerPrefs.SetInt("record", int.Parse(GameObject.FindWithTag("kol").GetComponent<TMPro.TextMeshProUGUI>().text));
                        record.GetComponent<TMPro.TextMeshPro>().text = "Train here --->" + "/n" + "Record:" + PlayerPrefs.GetInt("record");
                    }
        ...
        {
            fly = true;
            rb.AddForce(Vector3.up * jumpForce * 3f);
            rb.AddForce(orientation.transform.forward * jumpForce * 0.4f);
        ...
        }else if(collision.gameObject.tag == "win")
        {
            if (SceneManager.GetActiveScene().name == "tutor")
            {
                ...
                {
                    PlayerPrefs.SetInt("time1",(int)getTime.GetComponent<for_timer>().secondsCount);
                    PlayerPrefs.SetInt("level", 1);
                    SceneManager.LoadScene("first_level");
                }
                else
                {
                     if (PlayerPrefs.GetInt("time1") > (int)getTime.GetComponent<for_timer>().secondsCount)
                     {
                            PlayerPrefs.SetInt("time1", (int)getTime.GetComponent<for_timer>().secondsCount);
                     }
                    SceneManager.LoadScene("first_level");
            ...
            if (SceneManager.GetActiveScene().name == "first_level")
            {
                ...
                {
                    PlayerPrefs.SetInt("time2", (int)getTime.GetComponent<for_timer>().secondsCount);
                    PlayerPrefs.SetInt("level", 2);
                }
                else
                {
                    if (PlayerPrefs.GetInt("time2") > (int)getTime.GetComponent<for_timer>().secondsCount)
                    {
                        PlayerPrefs.SetInt("time2", (int)getTime.GetComponent<for_timer>().secondsCount);
                    }
                }
            ...
            if (SceneManager.GetActiveScene().name == "second_level")
            {
                ...
                {
                    PlayerPrefs.SetInt("time3", (int)getTime.GetComponent<for_timer>().secondsCount);
                    PlayerPrefs.SetInt("level", 3);
                }
                else
                {
                    if (PlayerPrefs.GetInt("time3") > (int)getTime.GetComponent<for_timer>().secondsCount)
                    {
                        PlayerPrefs.SetInt("time3", (int)getTime.GetComponent<for_timer>().secondsCount);
                    }
                }
